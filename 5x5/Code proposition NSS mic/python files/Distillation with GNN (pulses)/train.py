"""
train.py — Full training pipeline (per-PULSE approach)

Mirrors ../Distillation with GNN/train.py but consumes the pulse-level cache
built by this workspace's preprocess.py (multi-primary labels per pulse).
"""

import os, random, sys, pickle, json, math
import numpy as np
import torch
...
def main():
    ...
    metrics_log_path = os.path.join(script_dir, "logs", "train_metrics.jsonl")
    os.makedirs(os.path.dirname(metrics_log_path), exist_ok=True)
    open(metrics_log_path, "w").close()  # reset

    orig_train_one_epoch = trainer.train_one_epoch
    orig_validate        = trainer.validate
    ep_state = {"n": 0, "tr_loss": float("nan")}

    def _train_wrap(loader, epoch):
        ep_state["n"] = epoch
        loss = orig_train_one_epoch(loader, epoch)
        ep_state["tr_loss"] = loss
        return loss

    def _val_wrap(loader):
        metrics = orig_validate(loader)
        rec = {"epoch": ep_state["n"], "train_loss": ep_state["tr_loss"]}
        for k, v in metrics.items():
            try:
                fv = float(v)
                rec[k] = fv if not math.isnan(fv) else None
            except Exception:
                rec[k] = None
    ...
    trainer.train_one_epoch = _train_wrap
    trainer.validate        = _val_wrap

    trainer.fit(dl_tr, dl_va, epochs=CFG["train"]["epochs"], early_patience=5)
    print(f"\nPer-epoch metrics written to {metrics_log_path}")
